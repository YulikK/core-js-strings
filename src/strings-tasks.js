...
function removeFirstOccurrences(str, value) {
  ...
  return firstOccurrences >= 0
    ? str.slice(0, firstOccurrences) +
        str.slice(firstOccurrences + value.length)
    : str;
}
...
function sumOfCodes(str) {
  ...
  if (typeof str === 'string' || str instanceof String) {
    sumOf = str
      .split('')
      .reduce((sum, char) => sum + char.charCodeAt(0), initialValue);
  }
  return sumOf;
}
...
function formatTime(minutes, seconds) {
  const minSrc = String(minutes).padStart(2, '0');
  const secSrc = String(seconds).padStart(2, '0');
  return `${minSrc}:${secSrc}`;
}
...
function countVowels(str) {
  ...
  let count = 0;
  if (str.length > 0) {
    count = str.split('').filter((abc) => vowels.includes(abc)).length;
  }
  return count;
}
...
function isPalindrome(str) {
  ...
  const strFilter = str
    .toLowerCase()
    .split('')
    .filter((abc) => !symbolArray.includes(abc))
    .join('');
  const reverseFilter = str
    .toLowerCase()
    .split('')
    .filter((abc) => !symbolArray.includes(abc))
    .reverse()
    .join('');
  return reverseFilter === strFilter;
}
...
function findLongestWord(sentence) {
  ...
  const lengthArray = sentenceArray.map((el) => el.length);
  const maxWorld = Math.max(...lengthArray);
  const ind = lengthArray.indexOf(maxWorld);
  return sentenceArray[ind];
}
...
function reverseWords(str) {
  const strArray = str.split(' ').map((el) => el.split('').reverse().join(''));
